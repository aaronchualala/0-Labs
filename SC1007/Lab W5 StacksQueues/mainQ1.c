...
int palindrome(Queue *wordPtr)
{
    ...
    ListNode *currentLetter;
    Stack wordStack;
    int i;
    trimQueue(wordPtr);
    currentLetter = wordPtr->head;
    for (i = 1; i <= floor(wordPtr->size / 2); i++)
    {
        push(&wordStack, currentLetter->item);
    ...
    while (!isEmptyStack(wordStack))
    {
        if (tolower(peek(wordStack)) != tolower(currentLetter->item))
        {
            return 0;
        }
        currentLetter = (currentLetter->next);
